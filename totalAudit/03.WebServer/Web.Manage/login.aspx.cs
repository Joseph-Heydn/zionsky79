...
namespace Web.Manage {
	public partial class login : BasePage {
		...
		private void fnAuthLogin() {
			...
			if ( vReturn == 0 ) {
				WebLog.WriteLog.fnActionLogWrite(fnHostIp(), gGameNo, AuthInfo.pAccountNo, 0, vReturn, "Login");
				Response.Redirect(pReturn.Value);
			} else {
				WebLog.WriteLog.fnActionLogWrite(fnHostIp(), gGameNo, 0, 0, vReturn, "Login");
