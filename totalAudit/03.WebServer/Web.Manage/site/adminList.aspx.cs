	...
	public partial class adminList : BasePage {
		...
		protected void fnDisplayAdminList() {
			try {
				...
				if ( vResult.Rows.Count == 0 ) {
					string vMessage = $"{fnLabelText("msg_Error_01")} [{vResult.Rows.Count}]";
					fnMessageText(vMessage);
				}
				...
				string vMessage = $"{fnLabelText("msg_Error_01")} [{e.Message}]";
				fnMessageText(vMessage);
			}
		}
		...
		protected void fnCreateUser() {
			...
			if ( oReturnNo == 0 ) {
				...
				string vMessage = $"{fnLabelText("msg_Error_01")} [{oReturnNo}]";
				fnMessageText(vMessage);
			}
		}
