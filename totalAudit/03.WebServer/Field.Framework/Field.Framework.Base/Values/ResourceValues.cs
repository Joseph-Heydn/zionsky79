...
//	private const string C_CLASS_VERSION = "1.0.0";
	public const string C_PATH_COMMON = "Resources.SiteCommon";
	public const string C_PATH_ENV = "Resources.SiteEnvironment";
	public const string C_PATH_MSG = "Resources.SiteMessage";
	public const string C_PATH_ERRMSG = "Resources.SiteErrorMessage";
	...
	private static string AppResourceValue(string pPath, string pKey) {
		...
		//string sValue = (string)System.Web.HttpContext.GetGlobalResourceObject(psPath, psKey);
		...
		// MEMO: ASP.NET 1.0 리소스 동적호출방법 (사용자등록 리소스파일) - [sPath: 네임스페이스경로지정]
		//oResValues.moResource = new System.Resources.ResourceManager(sPath, Assembly.GetExecutingAssembly());
		//--------------------------------------------------------------------------------------------------------------
	}
	...
	public static void InitContentPageResource(ContentPlaceHolder pContent, string pPath) {
		for ( int i = 0; i < pContent.Controls.Count; ++i ) {
			Control vControl = pContent.Controls[i];

		//	if ( vControl is UserControl ) { }
			if ( vControl is LiteralControl ) {
				continue;
			}
			...
			if ( vControl is Panel ) {
				for ( int m = 0; m < vControl.Controls.Count; ++m ) {
					Control vPanSub = vControl.Controls[m];
					SetControlsResource(vPanSub, pPath);
				}
			} else {
				SetControlsResource(vControl, pPath);
			}
		}
	}
