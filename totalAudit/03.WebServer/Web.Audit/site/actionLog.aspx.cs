	...
	public partial class actionLog : BasePage {
		...
		protected void fnInitAdminList() {
			try {
				...
				if ( oDTO.oReturnNo != 0 ) {
					string vMessage = string.Format("{0} [{1}]", fnLabelText("msg_Error_01"), Convert.ToString(oDTO.oReturnNo));
		...
		protected void fnInitDrpMenu() {
			...
			foreach ( DataRow vRow in vResult.Rows ) {
				...
				vNewRow["cMenuName"]	= string.Format("{0} > {1}", vGroupName, vRow["cMenuName"]);
		...
		private void fnDetailBind(ref RepeaterItem pItem) {
			...
			litMenuNo.Text		= string.Format("({0})", ((Literal) pItem.FindControl("cMenuNo")).Text);
