<%@ Page
	...
	Inherits="Web.Manage.web.board.list"
%>
...
<asp:Content ContentPlaceHolderId="cpScript" runat="server">
	<script type="text/javascript">
		...
		// 글 작성
		function fnWrite() {
			var vMoveUrl = "write.aspx?m={0}&r={1}";
			$(location).attr("href", String.format(vMoveUrl, gMenuNo, gReturn));
		}

		// 페이지 이동
		function fnMovePage(pPageNo, pRowCnt, pLastNo, pLastVo, pJumpNo, pIsNext) {
			var vPage = String.format(
					"list.aspx?m={0}&p={1}&c={2}&l={3}&j={4}&n={5}"
				,	gMenuNo
				,	pPageNo
				,	pRowCnt
				,	pLastNo
...
<asp:Repeater id="repMediaList" runat="server">
	<ItemTemplate>
				<ul>
					<li class="thum_img">
						<a href="javascript:fnView(<%# Eval("cWriteNo") %>);">
							<img src="<%# fnMovieImageUrl(Eval("cExtLink")) %>" style="width:200px;" alt=""/>
						</a>
					</li>
					<li class="thum_tit">
                        <a href="javascript:fnView(<%# Eval("cWriteNo") %>);">
	...
	<ItemTemplate>
                <div class="announce">
                    ...
                    <div class="announce_img">
                        <a href="javascript:fnView(<%# Eval("cWriteNo") %>);">
	                        <img src="<%# fnImageUrl(Eval("cFolder"), Eval("cImage"), Eval("cExts")) %>" alt=""/>
                        </a>
                    </div>
