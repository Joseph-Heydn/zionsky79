...
function fnSetEditor() {
	tinymce.init({
		selector : window.gObjectId.txtContents3
	,	plugins : ["advlist autolink lists link image preview media table paste textcolor colorpicker textpattern imagetools searchreplace code fullscreen"]
	,	toolbar : "table | bold italic | forecolor backcolor | alignleft aligncenter alignright | bullist numlist outdent indent | link media searchreplace"
...
//	window.gObjectId.txtContents.val(vContents);
	tinymce.triggerSave();
	window.gObjectId.txtOnlyText.val(tinymce.getContent({format : "text"}));

	if ( window.gObjectId.txtContents.val().length < 1 ) {
		alert(window.gMessage.Alert_04);
		return false;
	}

	// 동영상인 경우 패스
	if ( $(window.gObjectId.lstAttachList) != undefined ) {
		fnGetImageList();
	}
...
// 리스트
function fnList() {
	$(location).attr("href", window.pReturn);
}
