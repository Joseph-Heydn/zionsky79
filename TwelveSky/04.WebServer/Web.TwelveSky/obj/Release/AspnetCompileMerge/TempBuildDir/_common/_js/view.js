var vParams = {
		pMenuNo		: window.gObjectId.pMenu
	,	pWriteNo	: window.gObjectId.pWrite
	};

// 글 수정
function fnEdit() {
	if ( fnCheckAuth() ) {
		var vMovePage = "write.aspx?pMenu={0}&pWrite={1}&pReturn={2}";
		$(location).attr("href", String.format(vMovePage, vParams.pMenuNo, vParams.pWriteNo, window.gObjectId.pReturn));
	}
}
...
// 리스트
function fnList() {
	$(location).attr("href", window.gObjectId.pReturn);
}
...
function fnRegistClick() {
	if ( window.fnCheckTag($("#txtComment").val()) ) {
		alert(window.gMessage.pError5);
		return;
	}
			...
			switch ( pData.d ) {
				...
				default:
					...
					switch ( vReturn[0] ) {
						case "0":
							if ( pType === 1 ) {
								alert(window.gMessage.pRepot1);
								$("#spRecmdCnt").text(vReturn[1]);
							} else {
								alert(window.gMessage.pRepot3);
							}
							break;
						case "6":
							alert(window.gMessage.pError2);
							break;
						case "7":
							alert(window.gMessage.pError4);
							break;
						default:
							alert(window.gMessage.pError1);
					}
			}
		};
