...
function fnSetCookie(pName, pValue, pDay) {
	...
	var vCookies = pName +"="+ escape(pValue) +"; path=/";
...
// 쿠키 생성 (초단위 종료)
function fnSetCookie2(pName, pValue, pDay) {
	var vExpire = new Date();
	// 한글 깨짐을 막기위해 escape(pValue)를 합니다.
	var vCookies = pName +"="+ escape(pValue) +"; path=/";
	vExpire.setSeconds(vExpire.getSeconds() + pDay);
...
function fnMoveBill(pPage) {
	$(location).attr("href", window.gBill + pPage);
}

function fnPolicy(pWrite) {
	$(location).attr("href", String.format("/web/policy.aspx?m=1000026&w={0}", pWrite));
}

function fnPolicyPop(pWrite) {
	var url = String.format("/web/policy.aspx?m=1000026&w={0}", pWrite);
