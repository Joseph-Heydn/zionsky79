...
{
    public partial class main_banner : System.Web.UI.Page
    {
...
-- ■ API          : " + Context.Request.Path + @" --
                ...
                if (!reader2.HasRows) throw new System.Exception(); //다음 SEQ를 가져오지 못함
                if (reader2.Read())
                {
                ...
                tmpSQL = @"
                        ...
                        VALUES
                            (" + SEQ + ", '" + RELT_DIV + "', " + NOW_SEQ + ",'" + FILE_NM + "', '" + CHNG_NM + "', '" + FILE_SIZE + "', '" + FILE_TYP + "', '" + FILE_DIV + "', '" + USER_ID + "', GETDATE())";
