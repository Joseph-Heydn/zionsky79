...
namespace _12sky2.resources.file {
	public partial class image : Page {
		...
		//	DbDataAdapter adapter = connect.Factory(ConfigurationManager.AppSettings["NAT_DATA"]).CreateDataAdapter();
			...
			try {
				var tmpSQL = @"SELECT SEQ, RELT_DIV, RELT_SEQ, FILE_NM, CHNG_NM, FILE_SIZE, FILE_TYP, FILE_DIV FROM T_FILE_INFO where SEQ = " +
					Request.QueryString["seq"].Trim() + "";
				...
				Trans.Rollback();
				SYS.Save_Log($"[{Request.Path} 에디터 image 불러오기 실패] {ex.Message}");
			} finally {
				cnn.Close();
			}
		}
