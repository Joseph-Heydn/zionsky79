...
" + ex.Message;
		...
		/*********************************************************************************************************************/
		[WebMethod(EnableSession = true)]
		public Exception insert(string USER_ID, string NICK_NM, string PSWD, string USE_AUTH, string NAT_CD, string SCRT_EMAIL,
			string EMAIL_RECV_YN) {
			string tmpSQL = null, msg = null;
			int NOW_SEQ = 0;
			string USER_NO = null;
...
-- ■ API          : " + Context.Request.Path + @" --
							...
							SELECT isnull('" + NAT_CD + @"'
									+ replicate('0', 11 - LEN(cast(cast(replace(MAX(USER_NO), '" + NAT_CD +
...
" + ex.Message;
		...
		/*********************************************************************************************************************/
		[WebMethod(EnableSession = true)]
		public Exception updateMberInfo(string BRDD, string SEX, string EMAIL_RECV_YN, string REPR_IMGE_PATH, string USER_ID,
			string USE_AUTH) {
			string tmpSQL = null, msg = null;
...
" + ex.Message;
		...
		/*********************************************************************************************************************/
		[WebMethod(EnableSession = true)]
		public bool login(string ID, string PSWD, string NAT_CD) {
			bool ret = false;
			string tmpSQL = null, msg = null;
			string SEQ = null;
...
-- ■ API          : " + Context.Request.Path + @" --
				...
				if ( reader.Read() ) {
					...
					FormsAuthenticationTicket authTicket = new FormsAuthenticationTicket(1, reader["USER_ID"].ToString().Trim(),
						DateTime.Now, DateTime.Now.AddMinutes(1440*7), isCookiePersistent, "Users");
					...
					userid.Domain = SYS.COOKIE_DOMAIN;
...
" + ex.Message;
		...
		/*********************************************************************************************************************/
		[WebMethod(EnableSession = true)]
		public bool findid(string NICK_NM) {
			bool ret = false;
			string tmpSQL = null, msg = null;
			string USER_ID = null;
...
" + ex.Message;
		...
		/*********************************************************************************************************************/
		[WebMethod(EnableSession = true)]
		public bool findpassword(string USER_ID) {
			bool ret = false;
			string tmpSQL = null, msg = null;
...
-- ■ API          : " + Context.Request.Path + @" --
				...
				string subject = @"
					Dear, <span>" + NICK_NM +
					@"</span><br>	You have requested a password.<br>Here is your User password.<br>
