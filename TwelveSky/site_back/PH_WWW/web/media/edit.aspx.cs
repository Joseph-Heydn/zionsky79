    ...
    public partial class edit : System.Web.UI.Page
    {
        ...
        private void getRead()
        {
            try
            {
                ...
                // 첨부파일 정보
                //-------------------------------------------------
                DataTable fileInfo = T_FILE_INFO.getList(Request.QueryString["DIV"].ToString(), Request.QueryString["SEQ"].ToString(), "");
                ...
                if (!fileInfo.Rows.Count.Equals(0))
                {
                    for (int i = 0; i < fileInfo.Rows.Count; i++)
                    {
                        ...
                        EDIT_FILE_LIST.InnerHtml += "[" + fileInfo.Rows[i]["FILE_DIV"].ToString() + "] ";
        ...
        protected void btn_save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                T_MEDIA_BORD.update(Request.QueryString["DIV"].ToString(), Request.QueryString["SEQ"].ToString(), _title.Value, tmpCntn, Session["USER_ID"].ToString(), Session["USER_NICK_NM"].ToString(), "N", null, FILE_SEQ.Value.Split(new char[] { ',' }), DEL_FILE_SEQ.Value.Split(new char[] { ',' }));
