...
namespace _12sky2.webservice
{
    ...
    public class T_COMMUNITY_BORD : System.Web.Services.WebService
    {
        /*********************************************************************************************************************/
        /* total count
        /*********************************************************************************************************************/
                ...
                (
                    ...
                    (
                        SELECT
                            ...
                            END AS CMMT_CNT
                            , CASE 
                                WHEN (SELECT COUNT(SEQ) FROM T_FILE_INFO WHERE RELT_DIV=A.BORD_DIV AND RELT_SEQ = A.SEQ AND FILE_DIV = 'FILE') > 0
                                THEN '" + SYS.ICON_FILE + @"'
                                ELSE ''
                            END as ICON_FILE
                            , CASE 
                            ...
                            END as ICON_NEW
                            , CASE 
                                WHEN (SELECT COUNT(SEQ) FROM T_LIKE WHERE RELT_DIV = A.BORD_DIV AND RELT_SEQ = A.SEQ AND LIKE_YN = 'N') >= 10
                                THEN 'n_like'
                                ELSE ''
                            END as N_LIKE_CSS
                        FROM T_COMMUNITY_BORD A
                        WHERE DEL_YN = 'N' 
                        AND BORD_DIV = '" + BORD_DIV + @"'
                        AND NAT_CD = '" + NAT_CD + @"'
                    ) A
...
" + ex.Message;
        ...
        //
        [WebMethod(EnableSession = true)]
        public DataTable getTopList(string BORD_DIV, string NAT_CD)
        {
            DataTable result = new DataTable("DataTable");
            string tmpSQL = null, msg = null;
...
" + ex.Message;
        ...
        //
        [WebMethod(EnableSession = true)]
        public DataTable getTop5List(string BORD_DIV, string NAT_CD)
        {
            DataTable result = new DataTable("DataTable");
            string tmpSQL = null, msg = null;
...
-- ■ API          : " + Context.Request.Path + @" --
            ...
            (
                SELECT                    
                    ...
                    , (SELECT COUNT(SEQ) 
                        FROM T_LIKE 
                        WHERE RELT_DIV = A.BORD_DIV
                        AND RELT_SEQ = A.SEQ 
                        AND LIKE_YN = 'Y'
                        AND CONVERT(VARCHAR(10), REG_DTM, 120) <= CONVERT(VARCHAR(10), GETDATE()-1, 120)
                    ) as LIKE_CNT
                FROM T_COMMUNITY_BORD A
                WHERE DEL_YN = 'N' 
                AND BORD_DIV = '" + BORD_DIV + @"'
...
-- ■ API          : " + Context.Request.Path + @" --
                ...
                SELECT
                    ...
                    , (SELECT COUNT(SEQ) FROM T_LIKE WHERE RELT_DIV = A.BORD_DIV AND RELT_SEQ = A.SEQ AND LIKE_YN = 'Y') as Y_LIKE
                    , (SELECT COUNT(SEQ) FROM T_LIKE WHERE RELT_DIV = A.BORD_DIV AND RELT_SEQ = A.SEQ AND LIKE_YN = 'N') as N_LIKE
                FROM T_COMMUNITY_BORD A
                WHERE DEL_YN = 'N' 
                AND BORD_DIV = '" + BORD_DIV + @"'
                AND NAT_CD = '" + NAT_CD + @"'
...
" + ex.Message;
        ...
        //
        [WebMethod(EnableSession = true)]
        public Exception insert(string BORD_DIV, string TITL, string CNTN, string USER_ID, string USER_NICK_NM, string TOP_POST_YN, string NAT_CD, string[] FILE_SEQ)
        {
...
-- ■ API          : " + Context.Request.Path + @" --
                            ...
                            SELECT ISNULL(MAX(SEQ)+1, 1) FROM T_COMMUNITY_BORD
                            ";
...
" + ex.Message;
        ...
        //
        [WebMethod(EnableSession = true)]
        public Exception update(string BORD_DIV, string SEQ, string TITL, string CNTN, string USER_ID, string USER_NICK_NM, string TOP_POST_YN, string[] FILE_SEQ, string[] DEL_FILE_SEQ)
        {
