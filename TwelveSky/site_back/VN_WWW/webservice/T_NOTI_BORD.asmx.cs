...
namespace _12sky2.webservice {
	...
	public class T_NOTI_BORD : WebService {
		/*********************************************************************************************************************/
		/* total count
        /*********************************************************************************************************************/
		//
                ...
                (
                    ...
                    (
                        SELECT
                            ...
                            , CASE 
		                        WHEN LEN(dbo.udf_StripHTML(CNTN)) > 360
		                        THEN SUBSTRING(dbo.udf_StripHTML(CNTN), 0, 360) + '... [more]'
		                        ELSE dbo.udf_StripHTML(CNTN)
	                        END AS CNTN2
                            , '' as IMG_SRC
                            , CASE 
...
" + ex.Message;
		...
		//
		[WebMethod(EnableSession = true)]
		public DataTable getList2(string BORD_DIV, string NAT_CD, int NOW_PAGE) {
			DataTable result = new DataTable("DataTable");
			string tmpSQL = null, msg = null;

			int LIST_CNT = int.Parse(SYS.LIST_EA);
			if ( BORD_DIV.Equals("announcement") )
				LIST_CNT = 5;
...
" + ex.Message;
		...
		//
		[WebMethod(EnableSession = true)]
		public DataTable getTop4List(string BORD_DIV, string NAT_CD) {
			DataTable result = new DataTable("DataTable");
			string tmpSQL = null, msg = null;
...
-- ■ API          : " + Context.Request.Path + @" --
                            ...
                            SELECT ISNULL(MAX(SEQ)+1, 1) FROM T_NOTI_BORD
                            ";
				...
				DbDataReader reader = command.ExecuteReader();
				if ( !reader.HasRows )
					throw new System.Exception(); //다음 SEQ를 가져오지 못함
				if ( reader.Read() ) {
