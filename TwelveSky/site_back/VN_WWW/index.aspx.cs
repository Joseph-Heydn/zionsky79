	...
	public partial class index : Page {
		...
		private void setLogin() {
			try {
				...
				MY_PAGE.InnerHtml = SYS.is_admin(Page)
									? "<a href='/admin/' target='_blank'>Admin Page <span>GO ></span></a>"
									: "<a href='/web/mypage/profile.aspx'>My Page <span>GO ></span></a>";
			} catch ( Exception ex ) {
				SYS.Save_Log(ex.Message);
			}
		}
		...
		protected void btn_login_Click(object sender, EventArgs e) {
			if ( SYS.is_null(ID.Text) ) {
				SYS.Javascript(Page, $"alert('{fnLabelText("msg_Error_01")}');");
				ID.Focus();
				return;
			}
			if ( SYS.is_null(PSWD.Text) ) {
				SYS.Javascript(Page, $"alert('{fnLabelText("msg_Error_02")}');");
				PSWD.Focus();
				return;
			}
			...
			if ( T_MBER.login(ID.Text, PSWD.Text, NAT_CD) ) {
				...
				SYS.Javascript(Page, $"alert('{fnLabelText("msg_Error_03")}');");
				PSWD.Focus();
			}
		}
