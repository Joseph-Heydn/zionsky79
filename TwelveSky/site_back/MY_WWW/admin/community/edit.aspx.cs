...
namespace _12sky2.admin.community {
	public partial class edit : Page {
		readonly webservice.T_COMMUNITY_BORD T_COMMUNITY_BORD = new webservice.T_COMMUNITY_BORD();
		...
		protected void Page_Load(object sender, EventArgs e) {
			...
			if ( !IsPostBack ) {
				admin_left.setSUB_TITL = Request.QueryString["DIV"];
		...
		private void getRead() {
			try {
				DataTable info = T_COMMUNITY_BORD.getRead(Request.QueryString["DIV"], NAT_CD, Request.QueryString["SEQ"]);
				...
				// 첨부파일 정보
				//-------------------------------------------------
				DataTable fileInfo = T_FILE_INFO.getList(Request.QueryString["DIV"], Request.QueryString["SEQ"], "");
				...
				for ( int i = 0; i < fileInfo.Rows.Count; i++ ) {
					...
					EDIT_FILE_LIST.InnerHtml += "<div id='file_" + fileInfo.Rows[i]["SEQ"] + "'>";
					EDIT_FILE_LIST.InnerHtml += "[" + fileInfo.Rows[i]["FILE_DIV"] + "] ";
					EDIT_FILE_LIST.InnerHtml += fileInfo.Rows[i]["FILE_LINK"].ToString();
					EDIT_FILE_LIST.InnerHtml += " (" + fileInfo.Rows[i]["FILE_SIZE"] + "Mb) ";
					EDIT_FILE_LIST.InnerHtml += " <a href='#' onclick='fileDelete(\"" + fileInfo.Rows[i]["SEQ"] + "\", \"" + fileInfo.Rows[i]["FILE_DIV"] + "\");'>delete</a>";
		...
		protected void btn_save_Click(object sender, EventArgs e) {
			try {
				...
				T_COMMUNITY_BORD.update(Request.QueryString["DIV"], Request.QueryString["SEQ"], _title.Value,
					tmpCntn, Session["USER_ID"].ToString(), Session["USER_NICK_NM"].ToString(), "N", FILE_SEQ.Value.Split(','), DEL_FILE_SEQ.Value.Split(','));

				string[] REMOVE = {};
				SYS.Javascript(Page, "location.href = '" + SYS.makeURL(this, "read.aspx", REMOVE) + "';");
			} catch ( Exception ) {
				//SYS.Save_Log(ex.Message);
