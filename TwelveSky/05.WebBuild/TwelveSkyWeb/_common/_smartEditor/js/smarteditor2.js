/**
 * SmartEditor2 NHN_Library:SE2.3.10.O11329:SmartEditor2.0-OpenSource
 * @version 11329
**/
...
nhn.husky.SE_EditingArea_TEXT = jindo.$Class({
	...
	irToText: function(sHtml) {
		...
		sContent = sContent.replace(/<br>/gi, '\n');
	...
	**/
	...
	setIR: function(sIR) {
		...
		if ( (this.iframe.contentWindow.document.body.innerHTML).replace(/[\r\n\t\s]*/, "") === "" ) {
			if ( this.oApp.sLineBreaker !== "BR" ) {
				sCursorHolder = sCursorHolder +"<br/>";
			}
	...
	**/
	_filterPastedContents: function(sOriginalContent) {
		...
		var sMatch, // 판별식과 일치하는 부분
			...
			aMultiParagraphIndicator = [
				"BLOCKQUOTE", "DD", "DIV", "DL", "FORM", "H1", "H2", "H3", "H4", "H5", "H6",
...
nhn.husky.SE2M_Quote = jindo.$Class({
	...
	$ON_REGISTER_CONVERTERS: function() {
		this.oApp.exec("ADD_CONVERTER", [
			"DB_TO_IR", jindo.$Fn(function(sContents) {
		...
		this.oApp.exec("ADD_CONVERTER", [
			"IR_TO_DB", jindo.$Fn(function(sContents) {
	...
	_isBlankQuote: function(elParentQuote) {
		var elChild,
			...
			isBlank = function(oNode) {
				...
				if ( (oNode.tagName === "P" || oNode.tagName === "SPAN") && (isBlankText(oNode.innerHTML) || oNode.innerHTML === "<br>") ) {
					return true;
				}
...
nhn.husky.SE2M_TableEditor = jindo.$Class({
	...
	$ON_REGISTER_CONVERTERS: function() {
		...
		this.oApp.exec("ADD_CONVERTER_DOM", [
			"WYSIWYG_TO_IR", jindo.$Fn(function(elTmpNode) {
...
//			jindo.$Element(aCellInfo[0]).css("height", aCellInfo[2]);
	...
	_isOnBorder: function(wevE) {
		...
		if ( wevE.element.tagName != "TD" && wevE.element.tagName != "TH" ) {
			return false;
		}
	...
	if ( this._instance == null ) {
		this._instance = new (function() {
			...
			this.openWindow = function(oWinOpt) {
				...
				for ( var i in oWinOpt )
					if ( oWinOpt.hasOwnProperty(i) ) {
						op[i] = oWinOpt[i];
					}
...
nhn.husky.SE2M_ImgSizeRatioKeeper = jindo.$Class({
	...
	$ON_EVENT_EDITING_AREA_MOUSEUP: function(wev) {
		this._detachResizeEnd();
		if ( !wev.element || wev.element.tagName != "IMG" ) {
			return;
		}
